...
describe('Comprehensive System Integration Tests', () => {
    ...
    it('100時間の村発展シミュレーション', () => {
      ...
      for (let hour = 0; hour < 100; hour++) {
        const currentTime: GameTime = {
          currentTime: gameTime.currentTime + hour * 3600,
          deltaTime: 3600 // 1時間
        };
  ...
  describe('極限状況での安定性テスト', () => {
    it('全資源枯渇からの回復テスト', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const currentTime: GameTime = {
          currentTime: gameTime.currentTime + i * 3600,
          deltaTime: 3600
        };
      ...
      for (let i = 0; i < 20; i++) {
        const currentTime: GameTime = {
          currentTime: gameTime.currentTime + 100 + i * 3600,
          deltaTime: 3600
        };
      ...
      expect(village.population).toBeGreaterThan(0);
      expect(village.storage.food).toBeGreaterThan(0);
    });
    ...
    it('大規模村（人口1000）での性能テスト', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const currentTime: GameTime = {
          currentTime: gameTime.currentTime + i * 100,
          deltaTime: 100
        };
    ...
    it('システム全体のメモリリークテスト', () => {
      ...
      for (let cycle = 0; cycle < 1000; cycle++) {
        const currentTime: GameTime = {
          currentTime: gameTime.currentTime + cycle * 10,
          deltaTime: 10
        };
