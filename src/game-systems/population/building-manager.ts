...
export class BuildingManager {
  ...
  updateBuildings(village: Village, gameTime: GameTime): void {
    ...
    try {
      ...
      console.error(
        `村 (${village.x}, ${village.y}) の建物更新でエラー:`,
        error,
      );
      // エラー時はエラーハンドラーでリセット
  ...
  private calculateMaxBuildableBuildings(village: Village): number {
    ...
    const queueCapacity = Math.max(
      0,
      3 - village.economy.buildings.constructionQueue,
    );
  ...
  private constructBuildings(
    ...
    console.log(
      `村 (${village.x}, ${village.y}): ${buildCount}棟の建物建設を開始 (木材: ${totalWoodCost}, 鉱石: ${totalOreCost})`,
    );
  }
  ...
  private processConstructionQueue(village: Village, gameTime: GameTime): void {
    ...
    const completedBuildings = Math.min(
      maxCompletable,
      Math.floor(maxCompletable * completionRate),
    );
    ...
    if (completedBuildings > 0) {
      ...
      village.economy.stock.capacity =
        ...
        village.economy.buildings.count *
          this.config.storageCapacityPerBuilding;

      console.log(
        `村 (${village.x}, ${village.y}): ${completedBuildings}棟の建物が完成 (総建物数: ${village.economy.buildings.count})`,
      );
    }
  }
