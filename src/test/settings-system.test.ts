...
describe('統合設定システム', () => {
  ...
  describe('基本機能', () => {
    ...
    it('設定を部分的に更新できる', () => {
      ...
      // 他の設定は変更されていないことを確認
      expect(settings.resources.recoveryRate).toBe(DEFAULT_GAME_SETTINGS.resources.recoveryRate);
      expect(settings.supplyDemand.populationGrowthRate).toBe(DEFAULT_GAME_SETTINGS.supplyDemand.populationGrowthRate);
    });
  ...
  describe('設定検証機能', () => {
    ...
    it('論理的整合性をチェックする', () => {
      ...
      expect(updateResult.isValid).toBe(false);
      expect(updateResult.errors.some(e => e.field.includes('Threshold'))).toBe(true);
    });
    ...
    it('推奨範囲外の値に警告を出す', () => {
      ...
      expect(updateResult.warnings.some(w => w.field === 'depletionRate')).toBe(true);
    });
  });
  ...
  describe('設定統計', () => {
    ...
    it('エラーがある場合の統計を正しく計算する', () => {
      ...
      const validationResult = settingsManager.validateSettings(invalidSettings);
