...
describe("Resource Shortage Scenarios", () => {
  ...
  function createTestVillage(
    ...
    production: { food: number; wood: number; ore: number } = {
      food: 0,
      wood: 0,
      ore: 0,
    },
  ...
  describe("食料不足シナリオ", () => {
    it("軽度の食料不足 - 人口増加停止", () => {
      ...
      // 食料生産を消費より少し下回るように調整
      const consumption = populationManager.calculateFoodConsumption(
        village.population,
      );
      ...
      // 需給状況は不足または危機的になる
      expect(["balanced", "shortage", "critical"]).toContain(
    ...
    it("重度の食料不足 - 人口減少開始", () => {
      ...
      const shouldDecrease =
        populationManager.shouldPopulationDecrease(village);
    ...
    it("食料完全枯渇 - 人口急減と村の衰退", () => {
      ...
      village.economy.consumption.food =
        populationManager.calculateFoodConsumption(village.population);
      village.economy.supplyDemandStatus.food = "critical";
  ...
  describe("建設資源不足シナリオ", () => {
    ...
    it("建設資源の部分的不足 - 制限された建設", () => {
      ...
      const maxBuildable = Math.min(
        Math.floor(
          village.storage.wood / DEFAULT_SUPPLY_DEMAND_CONFIG.buildingWoodCost,
        ),
        Math.floor(
          village.storage.ore / DEFAULT_SUPPLY_DEMAND_CONFIG.buildingOreCost,
        ),
      );
      ...
      // 制限された数だけ建設される
      expect(village.economy.buildings.constructionQueue).toBeLessThanOrEqual(
        maxBuildable,
      );
    });
    ...
    it("建設資源枯渇による生産効率低下", () => {
      ...
      // 建物不足により生産効率が低いことを確認
      expect(lowEfficiencyProduction.food).toBeLessThan(
        highEfficiencyProduction.food,
      );
      expect(lowEfficiencyProduction.wood).toBeLessThan(
        highEfficiencyProduction.wood,
      );
      expect(lowEfficiencyProduction.ore).toBeLessThan(
        highEfficiencyProduction.ore,
      );
    });
  });
  ...
  describe("複合的な資源不足シナリオ", () => {
    ...
    it("資源不足の連鎖反応", () => {
      ...
      expect(village.economy.production.food).toBeLessThan(
        initialProduction * 1.5,
      ); // 建物ボーナスが少ない
    });
  ...
  describe("村間の資源不足格差シナリオ", () => {
    it("資源不足村と余剰村の識別", () => {
      const villages: Village[] = [
        ...
        createTestVillage(
          30,
          { food: 5, wood: 50, ore: 30 },
          { food: 5, wood: 20, ore: 15 },
        ),
        // 建設資源不足村
        createTestVillage(
          25,
          { food: 80, wood: 3, ore: 2 },
          { food: 25, wood: 2, ore: 1 },
        ),
        // 余剰村
        createTestVillage(
          20,
          { food: 200, wood: 100, ore: 80 },
          { food: 40, wood: 30, ore: 25 },
        ),
        // バランス村
        createTestVillage(
          15,
          { food: 50, wood: 30, ore: 20 },
          { food: 15, wood: 12, ore: 8 },
        ),
      ];
      ...
      villages.forEach((village) => {
        village.economy.consumption = {
          ...
          wood:
            ...
            DEFAULT_SUPPLY_DEMAND_CONFIG.buildingWoodCost,
          ore:
            village.economy.buildings.constructionQueue *
            DEFAULT_SUPPLY_DEMAND_CONFIG.buildingOreCost,
        };
        village.economy.supplyDemandStatus =
          supplyDemandBalancer.evaluateVillageBalance(village);
      });
      ...
      // 不足村の特定
      const foodShortageVillage = shortageVillages.find(
        (v) => v.economy.supplyDemandStatus.food === "critical",
      );
      expect(foodShortageVillage).toBeDefined();

      // 余剰村の特定
      const surplusVillage = surplusVillages.find(
        (v) =>
    ...
    it("供給可能性の評価", () => {
      const shortageVillage = createTestVillage(25, {
        food: 5,
        wood: 50,
        ore: 30,
      });
      ...
      const surplusVillage = createTestVillage(20, {
        food: 200,
        wood: 100,
        ore: 80,
      });
      ...
      const distantVillage = createTestVillage(15, {
        food: 150,
        wood: 80,
        ore: 60,
      });
  ...
  describe("極端な資源不足シナリオ", () => {
    ...
    it("資源生産ゼロ環境での村の運命", () => {
      ...
      // 生産ゼロの環境（枯渇した土地）
      const depletedMap: Tile[][] = Array(10)
        .fill(null)
        .map(() =>
          Array(10)
            .fill(null)
    ...
    it("瞬間的な大量消費による資源枯渇", () => {
      ...
      const woodConsumption =
        10 * DEFAULT_SUPPLY_DEMAND_CONFIG.buildingWoodCost;
      const oreConsumption = 10 * DEFAULT_SUPPLY_DEMAND_CONFIG.buildingOreCost;

      // 実際の消費可能量は制限される
      const actualWoodConsumption = Math.min(
        woodConsumption,
        village.storage.wood,
      );
      const actualOreConsumption = Math.min(
        oreConsumption,
        village.storage.ore,
      );
