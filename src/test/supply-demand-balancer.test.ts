...
describe('SupplyDemandBalancer', () => {
    ...
    testVillages = [
      // 余剰村（食料豊富、木材余剰）
      {
        ...
        economy: {
          ...
          supplyDemandStatus: { food: 'surplus', wood: 'surplus', ore: 'balanced' }
        },
        lastUpdateTime: 0,
      ...
      // 不足村（木材不足）
      {
        ...
        economy: {
          ...
          supplyDemandStatus: { food: 'balanced', wood: 'shortage', ore: 'balanced' }
        },
        lastUpdateTime: 0,
      ...
      // 危機村（全資源危機）
      {
        ...
        economy: {
          ...
          supplyDemandStatus: { food: 'critical', wood: 'critical', ore: 'critical' }
        },
        lastUpdateTime: 0,
  ...
  describe('calculateResourceBalance', () => {
    it('should categorize villages by resource balance - 要件 6.1', () => {
      const foodBalance = balancer.calculateResourceBalance(testVillages, 'food');
    ...
    it('should calculate correct net balance and stock days', () => {
      const woodBalance = balancer.calculateResourceBalance(testVillages, 'wood');
      
      const surplusVillage = woodBalance.surplusVillages.find(v => v.village === testVillages[0]);
      if (surplusVillage) {
        expect(surplusVillage.netBalance).toBe(9); // 15 - 6
        expect(surplusVillage.stockDays).toBeCloseTo(8.33); // 50 / 6
      }

      const shortageVillage = woodBalance.shortageVillages.find(v => v.village === testVillages[1]);
      if (shortageVillage) {
  ...
  describe('evaluateSupplyPossibility', () => {
    ...
    it('should sort suppliers by supply capacity', () => {
      ...
      for (let i = 1; i < suppliers.length; i++) {
        expect(suppliers[i-1].supplyCapacity).toBeGreaterThanOrEqual(suppliers[i].supplyCapacity);
      }
    });
  });
