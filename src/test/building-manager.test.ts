...
describe("BuildingManager", () => {
  ...
  describe("getBuildingStats", () => {
    it("正しい建物統計情報を返す", () => {
      ...
      expect(stats.buildingCost.wood).toBe(
        DEFAULT_SUPPLY_DEMAND_CONFIG.buildingWoodCost,
      );
      expect(stats.buildingCost.ore).toBe(
  ...
  describe("統合テスト", () => {
    it("人口増加に伴う建物建設の完全なフロー", () => {
      ...
      mockVillage.economy.buildings.targetCount =
        currentBuildings + queuedBuildings;
      ...
      // 建物数が増加
      expect(mockVillage.economy.buildings.count).toBe(
        currentBuildings + queuedBuildings,
      );
      expect(mockVillage.economy.buildings.constructionQueue).toBe(0);
    });
