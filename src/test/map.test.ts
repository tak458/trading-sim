    ...
    it("should generate tiles with valid properties", () => {
      ...
      map.forEach((row, y) => {
        row.forEach((tile, x) => {
          ...
          // 最大資源量の検証
          expect(tile.maxResources.food).toBeGreaterThanOrEqual(
            tile.resources.food,
          );
          expect(tile.maxResources.wood).toBeGreaterThanOrEqual(
            tile.resources.wood,
          );
          expect(tile.maxResources.ore).toBeGreaterThanOrEqual(
            tile.resources.ore,
          );
