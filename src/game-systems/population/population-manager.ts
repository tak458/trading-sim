...
export class PopulationManager {
  ...
  updatePopulation(village: Village, gameTime: GameTime): void {
    ...
    try {
      ...
      console.error(
        `村 (${village.x}, ${village.y}) の人口更新でエラー:`,
        error,
      );
      // エラー時はエラーハンドラーでリセット
  ...
  canPopulationGrow(village: Village): boolean {
    ...
    // 食料状況の確認
    const currentFoodConsumption = this.calculateFoodConsumption(
      village.population,
    );
    const futurePopulation = village.population + 1;
    const futureFoodConsumption =
      this.calculateFoodConsumption(futurePopulation);
    ...
    const isNotCritical =
      foodStatus !== "critical" && foodStatus !== "shortage";

    return (
      hasGrowthPotential &&
      hasSufficientFood &&
      hasPositiveFoodBalance &&
      isNotCritical
    );
  }
  ...
  shouldPopulationDecrease(village: Village): boolean {
    ...
    const currentConsumption = this.calculateFoodConsumption(
      village.population,
    );
    const isProductionInsufficient =
      village.economy.production.food < currentConsumption * 0.3;
    ...
    return (
      isAboveMinimum &&
      (isFoodDepleted || (isProductionInsufficient && isCriticalStatus))
    );
  }
  ...
  private increasePopulation(village: Village, gameTime: GameTime): void {
    ...
    // 食料の豊富さに基づく成長ボーナス
    const foodAbundance = Math.min(
      2.0,
      village.storage.food /
        (this.calculateFoodConsumption(village.population) * 10),
    );
  ...
  private decreasePopulation(village: Village, gameTime: GameTime): void {
    // 減少率に基づく確率的な人口減少
    const declineChance =
      this.config.populationDeclineRate * gameTime.deltaTime;
    ...
    const foodShortage = Math.max(
      ...
      2.0 -
        village.storage.food /
          Math.max(0.1, this.calculateFoodConsumption(village.population)),
    );
