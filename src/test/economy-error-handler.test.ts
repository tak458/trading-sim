...
describe('EconomyErrorHandler', () => {
  ...
  describe('データ整合性チェック', () => {
    ...
    it('負の人口を検出する', () => {
      ...
      expect(result.errors.some(e => e.message.includes('人口が範囲外'))).toBe(true);
    });
    ...
    it('範囲外の収集範囲を検出する', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.message.includes('収集範囲が範囲外'))).toBe(true);
    });
    ...
    it('経済データが存在しない場合を検出する', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.message.includes('経済データが存在しない'))).toBe(true);
    });
    ...
    it('範囲外の生産量を検出する', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.message.includes('foodの生産量が範囲外'))).toBe(true);
      expect(result.errors.some(e => e.message.includes('woodの生産量が範囲外'))).toBe(true);
    });
  });
  ...
  describe('村経済データのリセット', () => {
    ...
    it('経済データが存在しない場合に初期化する', () => {
      ...
      expect(testVillage.economy).toBeDefined();
      expect(testVillage.economy.buildings.count).toBe(0);
      expect(testVillage.economy.stock.capacity).toBe(DEFAULT_SUPPLY_DEMAND_CONFIG.baseStorageCapacity);
    });
  ...
  describe('エッジケース', () => {
    it('ストレージが存在しない村を処理する', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(e => e.message.includes('ストレージデータが存在しない'))).toBe(true);
    });
    ...
    it('人口履歴が存在しない村を処理する', () => {
      ...
      // エラーが発生しないことを確認
      expect(() => errorHandler.validateVillageEconomy(testVillage)).not.toThrow();
    });
