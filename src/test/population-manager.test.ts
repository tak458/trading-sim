...
describe('PopulationManager', () => {
  ...
  describe('calculateFoodConsumption', () => {
    it('要件 2.1: 人口に比例して食料消費量を計算する', () => {
      ...
      const expected10 = 10 * DEFAULT_SUPPLY_DEMAND_CONFIG.foodConsumptionPerPerson * 1.0; // 効率性ファクター込み
      ...
      // 効率性ファクターを考慮した期待値
      const expected20 = 20 * DEFAULT_SUPPLY_DEMAND_CONFIG.foodConsumptionPerPerson * 0.98;
    ...
    it('大きな人口では効率性ボーナスが適用される', () => {
      ...
      const expectedBase = 50 * DEFAULT_SUPPLY_DEMAND_CONFIG.foodConsumptionPerPerson;
      // 効率性により若干少なくなることを確認
  ...
  describe('getPopulationStats', () => {
    it('正確な人口統計情報を返す', () => {
      ...
      expect(stats.shouldDecline).toBe(populationManager.shouldPopulationDecrease(testVillage));
