...
describe("需給バランサー統合テスト", () => {
  ...
  describe("integrated supply-demand evaluation", () => {
    it("should update village economics and evaluate supply-demand correctly - 要件 6.1, 6.4", () => {
      ...
      const totalFoodVillages =
        comparison.food.surplusVillages.length +
    ...
    it("should identify supply-demand relationships between villages - 要件 6.2, 6.3", () => {
      ...
      const supplyDemandResult =
        balancer.identifySupplyDemandVillages(testVillages);
      ...
      expect(supplyDemandResult.resourceSpecificBalances).toHaveProperty(
        "food",
      );
      expect(supplyDemandResult.resourceSpecificBalances).toHaveProperty(
        "wood",
  ...
  describe("performance and scalability", () => {
    it("should handle multiple villages efficiently", () => {
      ...
      const supplyDemandResult =
        balancer.identifySupplyDemandVillages(manyVillages);
      ...
      expect(
        comparison.food.surplusVillages.length +
          ...
          comparison.food.shortageVillages.length +
          comparison.food.criticalVillages.length,
      ).toBe(manyVillages.length);

      expect(
        supplyDemandResult.shortageVillages.length +
          supplyDemandResult.surplusVillages.length +
          supplyDemandResult.criticalVillages.length,
      ).toBeLessThanOrEqual(manyVillages.length);
    });
  });
