...
describe('SupplyDemand設定システム', () => {
  ...
  describe('設定値の妥当性検証', () => {
    ...
    it('設定を更新できる', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(settingsManager.getSettings().supplyDemand.foodConsumptionPerPerson).toBe(0.8);
    });
    ...
    it('デフォルト値にリセットできる', () => {
      ...
      expect(settingsManager.getSettings().supplyDemand).toEqual(DEFAULT_SUPPLY_DEMAND_CONFIG);
    });
...
describe('グローバル設定管理', () => {
  ...
  it('グローバル設定を更新できる', () => {
    const result = updateGlobalSettings({ supplyDemand: { foodConsumptionPerPerson: 0.6 } });
    expect(result.isValid).toBe(true);
