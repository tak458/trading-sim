...
export function buildRoads(map: Tile[][], villages: Village[]): Road[] {
  ...
  // 既存道路を記録する2次元配列
  const roadMask: boolean[][] = Array.from({ length: map.length }, () =>
  ...
  for (const [villageA, villageB] of delaunayPairs) {
    ...
    if (!existingRoad) {
      // 既存道路を優遇するA*
      const path = astarPathWithRoad(
        map,
        { x: villageA.x, y: villageA.y },
        { x: villageB.x, y: villageB.y },
        roadMask,
      );
...
function getTileCost(tile: Tile, map: Tile[][], x: number, y: number): number {
  ...
  switch (tile.type) {
    case "water":
      return Infinity; // 通行不可
    case "road":
      return ROAD_COST; // 既存の道は極端に安い
    case "land":
      return near ? 1 - NEAR_ROAD_BONUS : 1;
    case "forest":
      return near ? 2 - NEAR_ROAD_BONUS : 2;
    case "mountain":
      return near ? 3 - NEAR_ROAD_BONUS : 3;
  }
}

// 既存道路を優遇するA*探索
function astarPathWithRoad(
  map: Tile[][],
  ...
  while (open.length > 0) {
    ...
    for (const [dx, dy] of [
      ...
      const tentativeG =
        (gScore.get(key(current.x, current.y)) ?? Infinity) + cost;
