...
import { BuildingManager, GameTime } from '../building-manager';
...
describe('BuildingManager 統合テスト', () => {
  ...
  describe('経済システムとの統合', () => {
    ...
    it('建物完成後にストック容量が増加する', () => {
      ...
      // 時間を大幅に進めて建設完了
      const longGameTime: GameTime = { currentTime: 200, deltaTime: 15.0 };
  ...
  describe('完全な村シミュレーション', () => {
    it('時間経過による村の発展シミュレーション', () => {
      ...
      // 複数回の時間経過をシミュレート
      for (let i = 0; i < 10; i++) {
        const currentTime: GameTime = { currentTime: 100 + i * 5, deltaTime: 5.0 };
    ...
    it('資源枯渇シナリオでの建設停止', () => {
      ...
      // 複数回更新しても建設されない
      for (let i = 0; i < 5; i++) {
        const currentTime: GameTime = { currentTime: 100 + i * 2, deltaTime: 2.0 };
        economyManager.updateVillageEconomy(mockVillage, currentTime, mockMap);
        buildingManager.updateBuildings(mockVillage, currentTime);
      }
