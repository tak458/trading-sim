...
import { VillageEconomy, SupplyDemandConfig, DEFAULT_SUPPLY_DEMAND_CONFIG } from './village-economy';

/**
 * エラータイプの定義
 */
export type EconomyErrorType = 'calculation' | 'data_integrity' | 'state_inconsistency' | 'validation';
...
export class EconomyErrorHandler {
  ...
  correctInvalidValues(village: Village): boolean {
    ...
    try {
      // 基本データの修正
      correctionsMade = this.correctBasicData(village, villageId) || correctionsMade;
      
      // 経済データの修正
      correctionsMade = this.correctEconomyData(village, villageId) || correctionsMade;
      
      // ストレージデータの修正
      correctionsMade = this.correctStorageData(village, villageId) || correctionsMade;
   ...
   * @returns 村固有のエラーログ
   */
  getVillageErrorLog(villageId: string, maxEntries: number = 20): EconomyError[] {
  ...
  getErrorStatistics(): {
    ...
    this.errorLog.forEach(error => {
      ...
      errorsByVillage[error.villageId] = (errorsByVillage[error.villageId] || 0) + 1;
  ...
  private validateBasicData(village: Village, errors: EconomyError[], warnings: string[], villageId: string): void {
    // 人口の検証
    ...
    // 収集範囲の検証
    if (!this.isInRange(village.collectionRadius, this.ranges.collectionRadius)) {
      errors.push(this.createError(
        villageId,
        'data_integrity',
  ...
  private validateEconomyData(village: Village, errors: EconomyError[], warnings: string[], villageId: string): void {
    if (!village.economy) {
      errors.push(this.createError(
        villageId,
        'state_inconsistency',
    ...
    if (village.economy.production) {
      Object.entries(village.economy.production).forEach(([resource, value]) => {
        if (!this.isInRange(value, this.ranges.production)) {
          errors.push(this.createError(
            villageId,
            'data_integrity',
    ...
    if (village.economy.consumption) {
      Object.entries(village.economy.consumption).forEach(([resource, value]) => {
        if (!this.isInRange(value, this.ranges.consumption)) {
          errors.push(this.createError(
            villageId,
            'data_integrity',
    ...
    if (village.economy.buildings) {
      if (!this.isInRange(village.economy.buildings.count, this.ranges.buildings)) {
        errors.push(this.createError(
          villageId,
          'data_integrity',
  ...
  private validateRelationships(village: Village, errors: EconomyError[], warnings: string[], villageId: string): void {
    ...
    if (village.storage && village.economy.stock) {
      if (Math.abs(village.storage.food - village.economy.stock.food) > 0.1) {
        warnings.push(`食料ストックの不整合: storage=${village.storage.food}, economy=${village.economy.stock.food}`);
      }
    }

    // 人口と建物数の関係性
    if (village.economy.buildings) {
      const expectedBuildings = Math.floor(village.population * this.config.buildingsPerPopulation);
      const actualBuildings = village.economy.buildings.count;
      
      if (actualBuildings > village.population) {
        warnings.push(`建物数が人口を超過: 人口=${village.population}, 建物=${actualBuildings}`);
      }
    }
  }

  private validateCalculationResults(village: Village, errors: EconomyError[], warnings: string[], villageId: string): void {
    if (!village.economy) return;

    // 生産量と消費量の妥当性
    if (village.economy.production && village.economy.consumption) {
      Object.keys(village.economy.production).forEach(resource => {
        const production = village.economy.production[resource as keyof typeof village.economy.production];
        const consumption = village.economy.consumption[resource as keyof typeof village.economy.consumption];
        
        if (consumption > production * 10) {
          warnings.push(`${resource}の消費量が生産量に対して過大: 生産=${production}, 消費=${consumption}`);
        }
      });
    }
  }
  ...
  private correctBasicData(village: Village, villageId: string): boolean {
    ...
    village.population = this.clampToRange(village.population, this.ranges.population);
    if (village.population !== originalPopulation) {
      this.logCorrection(villageId, 'population', originalPopulation, village.population);
      corrected = true;
    }

    // 収集範囲の修正
    const originalRadius = village.collectionRadius;
    village.collectionRadius = this.clampToRange(village.collectionRadius, this.ranges.collectionRadius);
    if (village.collectionRadius !== originalRadius) {
      this.logCorrection(villageId, 'collectionRadius', originalRadius, village.collectionRadius);
      corrected = true;
    }
  ...
  private correctEconomyData(village: Village, villageId: string): boolean {
    ...
    if (village.economy.production) {
      Object.keys(village.economy.production).forEach(resource => {
        ...
        village.economy.production[key] = this.clampToRange(original, this.ranges.production);
        if (village.economy.production[key] !== original) {
          this.logCorrection(villageId, `production.${resource}`, original, village.economy.production[key]);
          corrected = true;
        }
      });
    }
    ...
    if (village.economy.consumption) {
      Object.keys(village.economy.consumption).forEach(resource => {
        ...
        village.economy.consumption[key] = this.clampToRange(original, this.ranges.consumption);
        if (village.economy.consumption[key] !== original) {
          this.logCorrection(villageId, `consumption.${resource}`, original, village.economy.consumption[key]);
          corrected = true;
        }
      });
    }
    ...
    if (village.economy.buildings) {
      ...
      village.economy.buildings.count = this.clampToRange(originalCount, this.ranges.buildings);
      if (village.economy.buildings.count !== originalCount) {
        this.logCorrection(villageId, 'buildings.count', originalCount, village.economy.buildings.count);
        corrected = true;
      }

      // 建設キューの修正
      const originalQueue = village.economy.buildings.constructionQueue;
      village.economy.buildings.constructionQueue = Math.max(0, Math.min(10, originalQueue));
      if (village.economy.buildings.constructionQueue !== originalQueue) {
        this.logCorrection(villageId, 'buildings.constructionQueue', originalQueue, village.economy.buildings.constructionQueue);
        corrected = true;
      }
    }
  ...
  private correctStorageData(village: Village, villageId: string): boolean {
    ...
    Object.keys(village.storage).forEach(resource => {
      ...
      village.storage[key] = this.clampToRange(original, this.ranges.resources);
      if (village.storage[key] !== original) {
        this.logCorrection(villageId, `storage.${resource}`, original, village.storage[key]);
        corrected = true;
      }
    });
  ...
  private logError(error: EconomyError): void {
    ...
    // コンソールにも出力（開発時のデバッグ用）
    console.warn(`[EconomyError] ${error.villageId}: ${error.message} (${error.recoveryAction})`);
  }

  private logCorrection(villageId: string, field: string, originalValue: any, correctedValue: any): void {
    this.logError(this.createError(
      villageId,
      'data_integrity',
