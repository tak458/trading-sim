...
describe('Complete System Integration Tests', () => {
  describe('Full system integration with all features', () => {
    it('should integrate all resource systems seamlessly', () => {
      ...
      const initialPopulation = villages.reduce((sum, v) => sum + v.population, 0)
      ...
      for (let frame = 0; frame < simulationFrames; frame++) {
        ...
        try {
          ...
          // 神の介入のテスト（ランダムに実行）
          if (frame % 50 === 0) {
            const randomTile = map[Math.floor(Math.random() * mapSize)][Math.floor(Math.random() * mapSize)]
            resourceManager.divineIntervention(randomTile, 'food', randomTile.maxResources.food * 0.5)
          }
      ...
      // 統合結果の検証
      const finalPopulation = villages.reduce((sum, v) => sum + v.population, 0)
      performanceMetrics.totalVillageGrowth = finalPopulation - initialPopulation
      
      // パフォーマンス検証
      const averageUpdateTime = performanceMetrics.updateTimes.reduce((sum, time) => sum + time, 0) / performanceMetrics.updateTimes.length
      expect(averageUpdateTime).toBeLessThan(50) // 50ms以下
      expect(performanceMetrics.errorCount).toBeLessThan(simulationFrames * 0.01) // エラー率1%以下
      ...
      for (let y = 0; y < mapSize; y++) {
        for (let x = 0; x < mapSize; x++) {
          ...
          expect(tile.resources.food).toBeLessThanOrEqual(tile.maxResources.food)
          expect(tile.resources.wood).toBeLessThanOrEqual(tile.maxResources.wood)
      ...
      console.log('Integration test completed successfully:', {
        ...
        errorRate: ((performanceMetrics.errorCount / simulationFrames) * 100).toFixed(2) + '%',
    ...
    it('should handle stress conditions gracefully', () => {
      ...
      for (let frame = 0; frame < 200; frame++) {
        ...
        if (frame % 30 === 0) {
          for (let i = 0; i < 3; i++) {
            const randomTile = map[Math.floor(Math.random() * mapSize)][Math.floor(Math.random() * mapSize)]
            resourceManager.divineIntervention(randomTile, 'wood', randomTile.maxResources.wood) // 完全回復
          }
        }
      }
    ...
    it('should maintain smooth visual transitions', () => {
      ...
      // 全ての資源を完全に消耗させる
      resourceManager.harvestResource(testTile, 'food', testTile.resources.food)
      resourceManager.harvestResource(testTile, 'wood', testTile.resources.wood)
      ...
      for (let step = 0; step < recoverySteps; step++) {
        resourceManager.divineIntervention(testTile, 'food', stepAmount * (step + 1))
        ...
        if (step > 0) {
          ...
          // 透明度の変化が急激でないことを確認
          const opacityChange = Math.abs(currentState.opacity - prevState.opacity)
          expect(opacityChange).toBeLessThan(0.3)
          
          // 回復進行度が増加していることを確認
          expect(currentState.recoveryProgress).toBeGreaterThanOrEqual(prevState.recoveryProgress)
        }
      }
  ...
  describe('User experience optimization', () => {
    it('should provide responsive user feedback', () => {
      ...
      for (let y = 0; y < 5; y++) {
        for (let x = 0; x < 5; x++) {
          const tile = map[y][x]
          if (tile.maxResources.food > 0 || tile.maxResources.wood > 0 || tile.maxResources.ore > 0) {
            testTile = tile
            break
          }
        }
        if (testTile.maxResources.food > 0 || testTile.maxResources.wood > 0 || testTile.maxResources.ore > 0) {
          break
        }
      }
      ...
      const hasResources = testTile.maxResources.food > 0 || testTile.maxResources.wood > 0 || testTile.maxResources.ore > 0
      if (hasResources) {
        const visualState = resourceManager.getVisualState(testTile)
      ...
      // 回復操作
      if (hasResources) {
        resourceManager.divineIntervention(testTile, 'food', initialResources.food)
        resourceManager.divineIntervention(testTile, 'wood', initialResources.wood)
        resourceManager.divineIntervention(testTile, 'ore', initialResources.ore)
    ...
    it('should maintain consistent performance under load', () => {
      ...
      for (let frame = 0; frame < 100; frame++) {
        ...
        performanceMetrics.minUpdateTime = Math.min(performanceMetrics.minUpdateTime, updateTime)
      }
      
      const averageUpdateTime = performanceMetrics.updateTimes.reduce((sum, time) => sum + time, 0) / performanceMetrics.updateTimes.length
      const standardDeviation = Math.sqrt(
        performanceMetrics.updateTimes.reduce((sum, time) => sum + Math.pow(time - averageUpdateTime, 2), 0) / performanceMetrics.updateTimes.length
      )
