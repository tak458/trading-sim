...
import { PerformanceOptimizer, DEFAULT_OPTIMIZATION_CONFIG } from '../performance-optimizer';
...
describe('Performance Optimization Integration Tests', () => {
  ...
  describe('PerformanceOptimizer', () => {
    it('should handle large number of villages efficiently', async () => {
      ...
      const gameTime = { currentTime: 1000, deltaTime: 1.0 };
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      map[y][x] = {
        ...
        lastHarvestTime: { food: 0, wood: 0, ore: 0 }
      };
    }
  }
