...
export class UIScene extends Phaser.Scene {
      ...
      if (this.showVillageStatus && this.villageStatusUI) {
        ...
        if (mapScene) {
          ...
          if (economyManagers.economyManager) {
            ...
            const shouldUpdate =
              Date.now() - this.lastStatusUIUpdateTime > 2000; // 2秒間隔
            if (shouldUpdate) {
  ...
  setupInput(): void {
    ...
    this.input.keyboard?.on("keydown-I", () => {
      this.resourceInfoState.isDetailedMode =
        !this.resourceInfoState.isDetailedMode;
      const mapScene = this.scene.get("MapScene") as MapScene;
      if (mapScene) {
        mapScene.setResourceInfoState({
          isDetailedMode: this.resourceInfoState.isDetailedMode,
        });
      }
  ...
  createDivineUI() {
    ...
    // 全資源ボタン
    const allBtn = this.add
      .text(0, 160, "All Resources", {
        fontSize: "11px",
        fontFamily: "Arial",
        color: "#ffffff",
  ...
  updateDivineUI() {
    ...
    // 資源タイプボタンの色を更新
    const resourceButtons = this.divineUI.list.slice(
      9,
      13,
  ...
  createResourceInfoUI() {
    ...
    const modeText = this.add.text(
      10,
      30,
      "Mode: Basic (Press 'I' for detailed)",
      {
        fontSize: "11px",
        fontFamily: "Arial",
        color: "#cccccc",
    ...
    // 資源情報表示テキスト
    this.resourceInfoText = this.add.text(
      10,
      50,
      "Hover over a tile to see resource info",
      {
        fontSize: "11px",
        fontFamily: "Arial",
        color: "#ffffff",
  ...
  updateResourceInfoUI() {
    ...
    const modeLabel = this.resourceInfoState.isDetailedMode
      ? "Detailed"
      : "Basic";
  ...
  createTimeDisplayUI() {
    ...
    this.timeDisplay = this.add.container(
