...
describe('設定検証機能包括テスト', () => {
  ...
  describe('ResourceConfig 詳細検証テスト', () => {
    it('depletionRate の境界値テスト', () => {
      ...
      testCases.forEach(testCase => {
        ...
        if (testCase.shouldBeValid) {
          ...
          expect(result.errors.some(e => e.field === 'depletionRate')).toBe(true);
          expect(result.correctedSettings!.resources.depletionRate).toBeGreaterThanOrEqual(0);
          expect(result.correctedSettings!.resources.depletionRate).toBeLessThanOrEqual(1);
        }
      });
    });
    ...
    it('typeMultipliers の詳細検証', () => {
      ...
      const result1 = settingsManager.validateSettings(negativeMultiplierSettings);
      expect(result1.isValid).toBe(false);
      expect(result1.errors.some(e => e.field.includes('typeMultipliers.land.food'))).toBe(true);
      expect(result1.correctedSettings!.resources.typeMultipliers.land.food).toBeGreaterThanOrEqual(0);
      ...
      expect(result2.warnings.some(w => w.field.includes('typeMultipliers.land.food'))).toBe(true);
    });
  });
  ...
  describe('SupplyDemandConfig 詳細検証テスト', () => {
    it('閾値の論理的整合性テスト', () => {
      ...
      expect(corrected.criticalThreshold).toBeLessThan(corrected.shortageThreshold);
      expect(corrected.shortageThreshold).toBeLessThan(corrected.surplusThreshold);
    });
    ...
    it('ストック容量の検証', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.correctedSettings!.supplyDemand.baseStorageCapacity).toBeGreaterThan(0);
      expect(result.correctedSettings!.supplyDemand.storageCapacityPerBuilding).toBeGreaterThan(0);
    });
  });
  ...
  describe('GraphicsConfig 詳細検証テスト', () => {
    ...
    it('UI設定の検証', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.correctedSettings!.graphics.uiSettings.panelOpacity).toBeLessThanOrEqual(1.0);
      expect(result.correctedSettings!.graphics.uiSettings.panelOpacity).toBeGreaterThanOrEqual(0.0);
    });
  });
  ...
  describe('GameplayConfig 詳細検証テスト', () => {
    it('difficulty の検証', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(['easy', 'normal', 'hard', 'extreme']).toContain(result.correctedSettings!.gameplay.difficulty);
    });
    ...
    it('initialResources の検証', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.correctedSettings!.gameplay.initialResources.food).toBeGreaterThanOrEqual(0);
      expect(result.warnings.some(w => w.field.includes('initialResources.wood'))).toBe(true);
    });
    ...
    it('autoSaveInterval の検証', () => {
      ...
      testCases.forEach(testCase => {
        ...
        const hasWarning = result.warnings.some(w => w.field === 'autoSaveInterval');
  ...
  describe('複雑な設定組み合わせテスト', () => {
    it('建物コストと初期資源のバランス検証', () => {
      ...
      expect(result.isValid).toBe(true); // エラーではない
      expect(result.warnings.some(w => w.field === 'initialResources')).toBe(true);
    });
    ...
    it('資源消耗と回復のバランス検証', () => {
      ...
      expect(result.isValid).toBe(true); // エラーではない
      expect(result.warnings.some(w => w.field === 'depletionRate')).toBe(true);
    });
  ...
  describe('サニタイズ機能詳細テスト', () => {
    it('部分的な設定のサニタイズ', () => {
      ...
      const sanitized = settingsManager.sanitizeSettings(partialInvalidSettings);
      ...
      // 指定されていない設定はデフォルト値
      expect(sanitized.resources.recoveryDelay).toBe(DEFAULT_GAME_SETTINGS.resources.recoveryDelay);
      expect(sanitized.supplyDemand).toEqual(DEFAULT_GAME_SETTINGS.supplyDemand);
    });
    ...
    it('ネストした構造のサニタイズ', () => {
      ...
      expect(sanitized.resources.depletionRate).toBeGreaterThanOrEqual(0);
      expect(sanitized.resources.typeMultipliers.land.food).toBeGreaterThanOrEqual(0);
      expect(sanitized.graphics.cameraSettings.zoomMin).toBeLessThan(sanitized.graphics.cameraSettings.zoomMax);
    });
  });
  ...
  describe('エッジケースと特殊値テスト', () => {
    ...
    it('非常に大きな数値の処理', () => {
      ...
      expect(result.correctedSettings!.supplyDemand.baseStorageCapacity).toBeLessThan(1e6);
    });
    ...
    it('型変換が必要な値の処理', () => {
      ...
      const sanitized = settingsManager.sanitizeSettings(typeConversionSettings);
