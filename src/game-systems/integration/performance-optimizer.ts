...
export class PerformanceOptimizer {
   ...
   * @returns 更新されたかどうか
   */
  optimizedUpdate(villages: Village[], gameTime: GameTime, map: Tile[][]): boolean {
   ...
   * @param map マップデータ
   */
  private processBatchUpdate(villages: Village[], gameTime: GameTime, map: Tile[][]): void {
    // バッチキューの更新
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      try {
        ...
        console.warn(`Village update error for village at (${item.village.x}, ${item.village.y}):`, error);
      }
    }
   ...
   * @param map マップデータ
   */
  private updateSingleVillage(village: Village, gameTime: GameTime, map: Tile[][]): void {
    // 経済システム更新
  ...
  private updateBatchQueue(villages: Village[]): void {
    // 新しい村を追加
    const existingVillageIds = new Set(this.batchQueue.map(item => `${item.village.x},${item.village.y}`));
  ...
  shouldUpdateUI(updateType: 'general' | 'villageText' | 'status'): boolean {
    ...
    switch (updateType) {
      case 'general':
        return now - this.lastUIUpdateTime >= this.adaptiveConfig.uiUpdateInterval;
      case 'villageText':
        return now - this.lastVillageTextUpdateTime >= this.adaptiveConfig.villageTextUpdateInterval;
      case 'status':
        return now - this.lastStatusUIUpdateTime >= this.adaptiveConfig.statusUIUpdateInterval;
      default:
        return false;
    }
  }
  ...
  private shouldProcessBatch(): boolean {
    return Date.now() - this.lastBatchProcessTime >= this.adaptiveConfig.batchProcessingInterval;
  }
  ...
  private shouldCleanupMemory(): boolean {
    return Date.now() - this.lastMemoryCleanupTime >= this.config.memoryCleanupInterval;
  }
  ...
  private shouldPerformDynamicOptimization(): boolean {
    return this.config.enableDynamicOptimization && 
           Date.now() - this.lastPerformanceCheckTime >= this.config.performanceMonitoringInterval;
  }
  ...
  private performMemoryCleanup(villages: Village[]): void {
    try {
      ...
      villages.forEach(village => {
        if (village.populationHistory && village.populationHistory.length > this.config.maxHistoryLength) {
          village.populationHistory = village.populationHistory.slice(-this.config.maxHistoryLength);
        }
      });
  ...
  private performDynamicOptimization(): void {
    try {
      ...
      if (avgFPS < this.config.targetFPS * 0.8) {
        // パフォーマンスが悪い場合
        const newBatchSize = Math.max(1, this.adaptiveConfig.maxVillagesPerBatch - 1);
        const newUIInterval = Math.min(2000, this.adaptiveConfig.uiUpdateInterval + 100);
        const newBatchInterval = Math.min(500, this.adaptiveConfig.batchProcessingInterval + 50);
      ...
      } else if (avgFPS > this.config.targetFPS * 1.1 && avgUpdateTime < this.config.maxUpdateTime * 0.5) {
        // パフォーマンスが良い場合
        const newBatchSize = Math.min(20, this.adaptiveConfig.maxVillagesPerBatch + 1);
        const newUIInterval = Math.max(200, this.adaptiveConfig.uiUpdateInterval - 50);
        const newBatchInterval = Math.max(50, this.adaptiveConfig.batchProcessingInterval - 25);
      ...
      if (configChanged) {
        console.log('Performance optimization: Configuration adjusted based on performance metrics');
      }
   ...
   * @param map マップデータ
   */
  private performFallbackUpdate(villages: Village[], gameTime: GameTime, map: Tile[][]): void {
    // 最小限の更新処理
   ...
   */
  private calculateBatchEfficiency(targetBatchSize: number, actualBatchSize: number): void {
