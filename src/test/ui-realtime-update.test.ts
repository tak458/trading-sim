...
describe("UI Display and Real-time Update Tests", () => {
  ...
  describe("基本的なUI表示機能", () => {
    it("村の資源不足情報を正しく表示する", () => {
      const village = createTestVillage(5, 5, 30, {
        food: 5,
        wood: 50,
        ore: 30,
      });
    ...
    it("複数村の不足情報を整理して表示する", () => {
      ...
      villages[2].economy.supplyDemandStatus = {
        food: "surplus",
        wood: "surplus",
        ore: "surplus",
      };
    ...
    it("資源不足がない場合は適切なメッセージを表示する", () => {
      const village = createTestVillage(5, 5, 20, {
        food: 100,
        wood: 80,
        ore: 60,
      });
  ...
  describe("リアルタイム更新機能", () => {
    it("時間経過による資源状況変化を追跡する", () => {
      const village = createTestVillage(5, 5, 25, {
        food: 50,
        wood: 30,
        ore: 20,
      });
      ...
      // 更新が実行されることを確認
      expect(
        (mockScene.add.text as any).mock.calls.length,
    ...
    it("急激な状況変化への即座の対応", () => {
      const village = createTestVillage(5, 5, 40, {
        food: 200,
        wood: 100,
        ore: 80,
      });
  ...
  describe("パフォーマンステスト", () => {
    it("大量の村でのUI更新性能", () => {
      ...
      // 100村を作成
      for (let i = 0; i < 100; i++) {
        const village = createTestVillage(i % 10, Math.floor(i / 10), 10 + i, {
  ...
  describe("UI状態管理とエラーハンドリング", () => {
    it("不正な村データでもUI表示が破綻しない", () => {
      const invalidVillage = createTestVillage(5, 5, -10, {
        food: -50,
        wood: -30,
        ore: -20,
      });
