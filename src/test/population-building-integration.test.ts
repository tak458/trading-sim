...
describe("Population and Building Integration Tests", () => {
  ...
  describe("人口増加に伴う建物需要の変化", () => {
    it("人口増加時に建物目標数が自動的に増加する", () => {
      ...
      // 目標建物数が増加していることを確認
      expect(testVillage.economy.buildings.targetCount).toBeGreaterThan(
        initialTargetCount,
      );
    ...
    it("建物完成により収集範囲と生産能力が向上する", () => {
      ...
      testVillage.economy.stock.capacity =
      ...
      expect(testVillage.economy.production.food).toBeGreaterThan(
        initialProduction.food,
      );
      expect(testVillage.economy.production.wood).toBeGreaterThan(
        initialProduction.wood,
      );
      expect(testVillage.economy.production.ore).toBeGreaterThan(
  ...
  describe("食料消費による人口変化と建物への影響", () => {
    it("食料不足による人口減少時に建物目標数も減少する", () => {
      ...
      if (testVillage.population < 30) {
        ...
        expect(testVillage.economy.buildings.targetCount).toBe(
    ...
    it("食料状況改善による人口増加時に建物建設が追従する", () => {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        // 建設キューを時間経過で処理
        const constructionTime = createGameTime(
          gameTime.currentTime + i * 10,
          5.0,
        );
      ...
      expect(
        testVillage.economy.buildings.count +
  ...
  describe("建設資源不足による制約の連鎖効果", () => {
    ...
    it("建物不足により生産効率が低下し人口成長が制限される", () => {
      ...
      // 建物が多い方が生産効率が高いことを確認
      expect(productionWithManyBuildings.food).toBeGreaterThan(
        productionWithFewBuildings.food,
      );
      expect(productionWithManyBuildings.wood).toBeGreaterThan(
        productionWithFewBuildings.wood,
      );
      expect(productionWithManyBuildings.ore).toBeGreaterThan(
        productionWithFewBuildings.ore,
      );
    });
  });
  ...
  describe("時間経過による複合的な変化", () => {
    it("長期間の時間経過で人口と建物が協調して成長する", () => {
      ...
      expect(testVillage.economy.buildings.count).toBeGreaterThanOrEqual(
        initialBuildingCount,
      );

      // 人口と建物数の比率が適切に維持されていることを確認
      const expectedBuildingCount = Math.floor(
        testVillage.population *
          DEFAULT_SUPPLY_DEMAND_CONFIG.buildingsPerPopulation,
      );
      ...
      expect(
        Math.abs(actualBuildingCount - expectedBuildingCount),
  ...
  describe("極端な状況での統合動作", () => {
    it("人口爆発時の建物建設追従性能", () => {
      ...
      expect(actualBuildingCount).toBeGreaterThanOrEqual(
        expectedBuildingCount * 0.8,
      ); // 80%以上の追従
    });
