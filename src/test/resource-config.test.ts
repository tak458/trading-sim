...
describe("資源設定システム", () => {
  describe("デフォルト設定", () => {
    ...
    it("バランスの取れたデフォルト値を持つ", () => {
      ...
      expect(
        DEFAULT_RESOURCE_CONFIG.minRecoveryThreshold,
      ).toBeGreaterThanOrEqual(0);
      expect(DEFAULT_RESOURCE_CONFIG.minRecoveryThreshold).toBeLessThanOrEqual(
        1,
      );
    });
  });
  ...
  describe("Configuration Validation", () => {
    ...
    it("should reject depletionRate above 1.0", () => {
      ...
      expect(result.errors).toContain(
        "depletionRate must not exceed 1.0 (100%)",
      );
    });
    ...
    it("should reject invalid minRecoveryThreshold", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "minRecoveryThreshold must be non-negative",
      );
    });
    ...
    it("should reject negative typeMultipliers", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "typeMultipliers.land.food must be non-negative",
      );
    });
  ...
  describe("Configuration Sanitization", () => {
    it("should sanitize invalid values to defaults", () => {
      ...
      expect(sanitized.minRecoveryThreshold).toBe(
        DEFAULT_RESOURCE_CONFIG.minRecoveryThreshold,
      );
    });
    ...
    it("should sanitize typeMultipliers", () => {
      ...
      const sanitized = sanitizeResourceConfig(invalidConfig);
      expect(sanitized.typeMultipliers.land.food).toBe(
        DEFAULT_RESOURCE_CONFIG.typeMultipliers.land.food,
      );
  ...
  describe("Configuration Presets", () => {
    ...
    it("should get preset configuration by name", () => {
      ...
      expect(easyConfig?.depletionRate).toBeLessThan(
        DEFAULT_RESOURCE_CONFIG.depletionRate,
      );
      expect(easyConfig?.recoveryRate).toBeGreaterThan(
        DEFAULT_RESOURCE_CONFIG.recoveryRate,
      );
    });
  ...
  describe("ResourceManager Configuration Integration", () => {
    ...
    it("should sanitize invalid config during update", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(manager.getConfig().depletionRate).toBe(
        DEFAULT_RESOURCE_CONFIG.depletionRate,
      );
    });
