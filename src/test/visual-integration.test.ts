...
describe('Visual System Integration', () => {
  ...
  it('should show visual changes after village harvesting', () => {
    ...
    for (let y = Math.max(0, village.y - 2); y <= Math.min(map.length - 1, village.y + 2); y++) {
      for (let x = Math.max(0, village.x - 2); x <= Math.min(map[0].length - 1, village.x + 2); x++) {
        const tile = map[y][x];
    ...
    if (resourceTile.resources.food < resourceTile.maxResources.food ||
        resourceTile.resources.wood < resourceTile.maxResources.wood ||
        resourceTile.resources.ore < resourceTile.maxResources.ore) {
      expect(finalVisualState.opacity).toBeLessThanOrEqual(initialVisualState.opacity);
    }
  });
  ...
  it('should show recovery progress visually over time', () => {
    ...
    expect(recoveredState.recoveryProgress).toBeGreaterThan(depletedState.recoveryProgress);
  });
