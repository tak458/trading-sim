...
describe("BuildingManager 統合テスト", () => {
  ...
  describe("経済システムとの統合", () => {
    ...
    it("建物完成後にストック容量が増加する", () => {
      ...
      const expectedCapacity =
        ...
        mockVillage.economy.buildings.count *
          DEFAULT_SUPPLY_DEMAND_CONFIG.storageCapacityPerBuilding;

      expect(mockVillage.economy.stock.capacity).toBe(expectedCapacity);
      expect(mockVillage.economy.stock.capacity).toBeGreaterThan(
        initialCapacity,
      );
    });
  });
  ...
  describe("人口システムとの統合", () => {
    it("人口増加に伴って建物需要が増加する", () => {
      ...
      // 目標建物数が増加していることを確認
      expect(mockVillage.economy.buildings.targetCount).toBeGreaterThan(
        initialTargetCount,
      );
  ...
  describe("資源制約との統合", () => {
    ...
    it("資源状況改善後は建設が再開される", () => {
      ...
      // 建設が再開される
      expect(mockVillage.economy.buildings.constructionQueue).toBeGreaterThan(
        0,
      );
    });
  });
  ...
  describe("完全な村シミュレーション", () => {
    it("時間経過による村の発展シミュレーション", () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (
          mockVillage.economy.buildings.count >
          initialBuildings + buildingsBuilt
        ) {
          buildingsBuilt =
            mockVillage.economy.buildings.count - initialBuildings;
        }

        // 村の状態が健全であることを確認
        expect(mockVillage.population).toBeGreaterThanOrEqual(1);
        expect(mockVillage.economy.buildings.count).toBeGreaterThanOrEqual(
          initialBuildings,
        );
