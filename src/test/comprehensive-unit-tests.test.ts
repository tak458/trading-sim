...
import type { GameTime, ResourceInfo } from "../game-systems/shared-types";
  ...
  describe("VillageEconomyManager - 包括的テスト", () => {
    describe("生産能力計算の詳細テスト", () => {
      it("人口ボーナスが正しく適用される", () => {
        ...
        const smallProduction = economyManager.calculateProduction(
          smallVillage,
          resources,
        );
        ...
        const largeProduction = economyManager.calculateProduction(
          largeVillage,
          resources,
        );
      ...
      it("建物ボーナスが正しく適用される", () => {
        ...
        const lowProduction = economyManager.calculateProduction(
          fewBuildings,
          resources,
        );
        ...
        const highProduction = economyManager.calculateProduction(
          manyBuildings,
          resources,
        );
      ...
      it("収集範囲が生産量に影響する", () => {
        ...
        const narrowProduction = economyManager.calculateProduction(
          narrowRange,
          resources,
        );
        ...
        const wideProduction = economyManager.calculateProduction(
          wideRange,
          resources,
        );
      ...
      it("利用可能資源がゼロの場合は生産量もゼロ", () => {
        ...
        const production = economyManager.calculateProduction(
          testVillage,
          noResources,
        );
    ...
    describe("エラーハンドリングテスト", () => {
      it("不正なマップデータでもエラーを起こさない", () => {
        expect(() => {
          economyManager.updateVillageEconomy(
            testVillage,
            gameTime,
            null as any,
          );
  ...
  describe("PopulationManager - 包括的テスト", () => {
    describe("食料消費計算の詳細テスト", () => {
      ...
      it("極端な人口値でも安全に処理する", () => {
        ...
        expect(
          populationManager.calculateFoodConsumption(1000),
        ).toBeGreaterThan(0);
      });
    });
    ...
    describe("人口減少条件の詳細テスト", () => {
      it("食料完全枯渇時は減少する", () => {
        ...
        expect(populationManager.shouldPopulationDecrease(testVillage)).toBe(
          true,
        );
      });
    ...
    describe("人口更新の詳細テスト", () => {
      ...
      it("人口履歴が正しく更新される", () => {
        ...
        expect(
          testVillage.populationHistory[
            testVillage.populationHistory.length - 1
          ],
        ).toBe(testVillage.population);
      });

      it("人口履歴は最大10件まで保持される", () => {
        testVillage.populationHistory = Array(12)
          .fill(0)
          .map((_, i) => i + 1);
  ...
  describe("BuildingManager - 包括的テスト", () => {
    ...
    describe("建物更新の詳細テスト", () => {
      ...
      it("建設可能時は資源を消費して建設キューに追加", () => {
        ...
        expect(testVillage.economy.buildings.constructionQueue).toBeGreaterThan(
          0,
        );
    ...
    describe("建物統計情報テスト", () => {
      it("正確な統計情報を返す", () => {
        ...
        expect(stats.targetCount).toBe(
          testVillage.economy.buildings.targetCount,
        );
        expect(stats.constructionQueue).toBe(
          testVillage.economy.buildings.constructionQueue,
        );
        expect(stats.canBuild).toBe(true);
        expect(stats.buildingCost).toEqual(
          buildingManager.calculateBuildingCost(),
        );
