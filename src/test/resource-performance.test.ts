...
describe('Resource System Performance Tests', () => {
  ...
  describe('Stress Tests', () => {
    ...
    it('should handle maximum configuration stress test', () => {
      ...
      for (let y = 0; y < 25; y++) {
        for (let x = 0; x < 25; x++) {
          const tile = map[y][x]
          if (tile.resources.food >= 0 && tile.depletionState.food >= 0 && tile.depletionState.food <= 1) {
            validTiles++
          }
        }
      }
  ...
  describe('Benchmark Tests', () => {
    ...
    it('should benchmark visual state calculations', () => {
      const tiles: Tile[] = Array(10000).fill(null).map(() => ({
        height: Math.random(),
        type: 'land' as const,
        resources: { food: Math.random() * 100, wood: Math.random() * 100, ore: Math.random() * 100 },
        maxResources: { food: 100, wood: 100, ore: 100 },
        depletionState: { food: Math.random(), wood: Math.random(), ore: Math.random() },
        recoveryTimer: { food: 0, wood: 0, ore: 0 },
        lastHarvestTime: 0
      }))
