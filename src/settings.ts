 ...
 * 各設定項目の制約条件
 */
const RESOURCE_CONSTRAINTS: Record<keyof ResourceConfig, SettingsConstraints | null> = {
  ...
  minRecoveryThreshold: { min: 0, max: 1, recommended: { min: 0.05, max: 0.3 } },
  typeMultipliers: null // 複雑な構造のため個別検証
};

const SUPPLY_DEMAND_CONSTRAINTS: Record<keyof SupplyDemandConfig, SettingsConstraints> = {
  foodConsumptionPerPerson: { min: 0.1, max: 2.0, recommended: { min: 0.15, max: 0.5 } },
  populationGrowthRate: { min: 0.001, max: 0.1, recommended: { min: 0.01, max: 0.05 } },
  populationDeclineRate: { min: 0.001, max: 0.2, recommended: { min: 0.02, max: 0.1 } },
  buildingsPerPopulation: { min: 0.05, max: 1.0, recommended: { min: 0.08, max: 0.2 } },
  ...
  shortageThreshold: { min: 0.3, max: 0.95, recommended: { min: 0.6, max: 0.9 } },
  criticalThreshold: { min: 0.1, max: 0.6, recommended: { min: 0.2, max: 0.5 } },
  baseStorageCapacity: { min: 50, max: 500, recommended: { min: 80, max: 200 } },
  storageCapacityPerBuilding: { min: 5, max: 100, recommended: { min: 10, max: 50 } }
};
...
export class SettingsManager {
  ...
  private constructor(initialSettings?: PartialGameSettings) {
    ...
    if (!validationResult.isValid && validationResult.correctedSettings) {
      ...
      console.warn('初期設定に問題があったため修正しました:', validationResult.errors);
    }
  }
   ...
   * @returns 更新結果
   */
  updateSettings(partialSettings: PartialGameSettings): SettingsValidationResult {
   ...
   */
  addListener(category: keyof GameSettings, listener: (settings: GameSettings) => void): void {
   ...
   */
  removeListener(category: keyof GameSettings, listener: (settings: GameSettings) => void): void {
  ...
  validateSettings(settings: GameSettings): SettingsValidationResult {
    ...
    const correctedSettings: GameSettings = JSON.parse(JSON.stringify(settings));

    // 資源設定の検証
    this.validateResourceConfig(settings.resources, errors, warnings, correctedSettings.resources);
    
    // 需給設定の検証
    this.validateSupplyDemandConfig(settings.supplyDemand, errors, warnings, correctedSettings.supplyDemand);
    
    // 時間設定の検証
    this.validateTimeConfig(settings.time, errors, warnings, correctedSettings.time);
    
    // グラフィックス設定の検証
    this.validateGraphicsConfig(settings.graphics, errors, warnings, correctedSettings.graphics);
    
    // ゲームプレイ設定の検証
    this.validateGameplayConfig(settings.gameplay, errors, warnings, correctedSettings.gameplay);
   ...
   * @returns インポート結果
   */
  importSettings(jsonString: string): { success: boolean; error?: string; validationResult?: SettingsValidationResult } {
    try {
      ...
      return {
        success: validationResult.isValid || validationResult.correctedSettings !== undefined,
        validationResult
      };
    } catch (error) {
      return {
  ...
  private mergeWithDefaults(partialSettings: PartialGameSettings): GameSettings {
    return {
      resources: { ...DEFAULT_RESOURCE_CONFIG, ...partialSettings.resources },
      supplyDemand: { ...DEFAULT_SUPPLY_DEMAND_CONFIG, ...partialSettings.supplyDemand },
  ...
  private mergeSettings(current: GameSettings, partial: PartialGameSettings): GameSettings {
    return {
  ...
  private notifyListeners(oldSettings: GameSettings, newSettings: GameSettings): void {
    // 変更されたカテゴリを検出
    const changedCategories: (keyof GameSettings)[] = [];
    
    if (JSON.stringify(oldSettings.resources) !== JSON.stringify(newSettings.resources)) {
      changedCategories.push('resources');
    }
    if (JSON.stringify(oldSettings.supplyDemand) !== JSON.stringify(newSettings.supplyDemand)) {
    ...
    if (JSON.stringify(oldSettings.graphics) !== JSON.stringify(newSettings.graphics)) {
      changedCategories.push('graphics');
    }
    if (JSON.stringify(oldSettings.gameplay) !== JSON.stringify(newSettings.gameplay)) {
...
private validateResourceConfig(
    ...
    // 基本的な数値フィールドの検証
    const numericFields: (keyof ResourceConfig)[] = ['depletionRate', 'recoveryRate', 'recoveryDelay', 'minRecoveryThreshold'];
    
    numericFields.forEach(field => {
      const value = config[field] as number;
      const constraints = RESOURCE_CONSTRAINTS[field];
      
      if (constraints && this.validateNumericField('resources', field, value, constraints, errors, warnings)) {
        (corrected as any)[field] = this.correctNumericValue(value, constraints);
      }
    });

    // typeMultipliers の検証
    if (config.typeMultipliers) {
      const tileTypes: (keyof TileTypeMultipliers)[] = ['water', 'land', 'forest', 'mountain', 'road'];
  ...
  private validateSupplyDemandConfig(
    ...
    // 全フィールドの検証
    (Object.keys(SUPPLY_DEMAND_CONSTRAINTS) as (keyof SupplyDemandConfig)[]).forEach(field => {
      const value = config[field];
      const constraints = SUPPLY_DEMAND_CONSTRAINTS[field];
      
      if (this.validateNumericField('supplyDemand', field, value, constraints, errors, warnings)) {
        (corrected as any)[field] = this.correctNumericValue(value, constraints);
      }
    });
  ...
  private validateTimeConfig(
    ...
    numericFields.forEach(field => {
      ...
      if (constraints && this.validateNumericField('time', field, value, constraints, errors, warnings)) {
        (corrected as any)[field] = this.correctNumericValue(value, constraints);
      }
    });
    ...
    if (config.updateIntervals) {
      const intervalFields: (keyof TimeConfig['updateIntervals'])[] = ['resources', 'villages', 'trade', 'visuals'];
    ...
    if (config.cameraSettings) {
      ...
      if (zoomDefault < zoomMin || zoomDefault > zoomMax) {
        errors.push({
          ...
          value: zoomDefault,
          message: 'デフォルトズームは最小ズームと最大ズームの間である必要があります',
          suggestedValue: (zoomMin + zoomMax) / 2
        });
        corrected.cameraSettings.zoomDefault = (corrected.cameraSettings.zoomMin + corrected.cameraSettings.zoomMax) / 2;
      }
    }

    // UI設定の検証
    if (config.uiSettings) {
      if (config.uiSettings.panelOpacity < 0 || config.uiSettings.panelOpacity > 1) {
        errors.push({
          category: 'graphics',
          field: 'uiSettings.panelOpacity',
          value: config.uiSettings.panelOpacity,
          message: 'パネル透明度は0から1の範囲内である必要があります',
          suggestedValue: Math.max(0, Math.min(1, config.uiSettings.panelOpacity))
        });
        corrected.uiSettings.panelOpacity = Math.max(0, Math.min(1, config.uiSettings.panelOpacity));
      }
    }
  }
      ...
      resourceTypes.forEach(resourceType => {
        ...
        if (value < 0) {
          ...
          warnings.push({
            ...
            field: `initialResources.${resourceType}`,
            value,
            message: '初期資源量が1000を超えるとゲームバランスが崩れる可能性があります',
            suggestedValue: 100
          });
        }
      });
  ...
  private validateLogicalConsistency(
    ...
    if (effectiveTickRate > 10) {
      warnings.push({
        ...
        field: 'gameSpeed',
        value: gameSpeed,
        message: '実効ティック率が高すぎるとパフォーマンスに影響する可能性があります',
    ...
    if (wood < buildingWoodCost || ore < buildingOreCost) {
      warnings.push({
        ...
        suggestedValue: { wood: buildingWoodCost * 2, ore: buildingOreCost * 2 }
      });
    }
  }
  ...
  private validateNumericField(
    ...
    if (value < constraints.min || value > constraints.max) {
      errors.push({
        ...
        suggestedValue: Math.max(constraints.min, Math.min(constraints.max, value))
      });
      hasError = true;
    }
  ...
  private correctNumericValue(value: number, constraints: SettingsConstraints): number {
    if (!isFinite(value) || isNaN(value)) {
 ...
 * @returns SettingsManagerインスタンス
 */
export function getGlobalSettingsManager(initialSettings?: PartialGameSettings): SettingsManager {
 ...
 * @returns 更新結果
 */
export function updateGlobalSettings(partialSettings: PartialGameSettings): SettingsValidationResult {
 ...
 * @returns 適用結果
 */
export function applySettingsPreset(presetName: string): SettingsValidationResult | null {
